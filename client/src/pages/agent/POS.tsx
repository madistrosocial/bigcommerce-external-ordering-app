...
import {
  isPosCustomerCacheFresh,
  searchCachedPosCustomers,
  syncPosCustomerDirectory,
  toLocalPosCustomer,
} from "@/lib/pos-customer-cache";
...
export default function POSPage() {
  ...
  // Keep a lightweight searchable customer directory on the device. This is
  // intentionally separate from the live address/store-credit calls made
  // after a customer is selected.
  useEffect(() => {
    if (!currentUser) return;
    const scope = `user:${currentUser.id}`;
    const sync = () => {
      if (!isOfflineMode && navigator.onLine) {
        void syncPosCustomerDirectory(scope).catch(() => {});
      }
    };
    sync();
  ...
  // ── Customer search ───────────────────────────────────────────────────────
  useEffect(() => {
    const requestId = ++customerSearchSeqRef.current;
    ...
    customerDebRef.current = setTimeout(async () => {
      ...
      const scope = currentUser ? `user:${currentUser.id}` : "";
      const isCurrent = () => requestId === customerSearchSeqRef.current;
      try {
        const localResults = await searchCachedPosCustomers(customerSearch);
        if (!isCurrent()) return;
        if (localResults.length > 0) {
          setCustomerResults(localResults);
        ...
        if (isOfflineMode || !navigator.onLine) {
          if (localResults.length === 0) {
            ...
            toast({
              title: "No cached customers found",
              description: "Reconnect to refresh the customer directory.",
            });
          }
          return;
        }

        const cacheFresh = localResults.length > 0 && scope
          ? await isPosCustomerCacheFresh(scope)
          : false;
        if (cacheFresh) return;

        // A local miss also starts the device refresh. The PostgreSQL query
        // runs independently so a large initial device sync cannot block the
        // current search.
        if (scope) {
          void syncPosCustomerDirectory(scope).catch(() => {});
        }

        let mirrorResults: api.BigCommerceCustomer[] = [];
        let mirrorError: unknown = null;
        try {
          mirrorResults = await api.searchPosCustomersFromPostgres(customerSearch);
        } catch (error) {
          mirrorError = error;
        }
        if (!isCurrent()) return;
        ...
        if (mirrorResults.length > 0) {
          await saveLocalPosCustomers(mirrorResults.map((customer) => toLocalPosCustomer(customer)));
          if (!isCurrent()) return;
          setCustomerResults(mirrorResults);
          setShowCustomerDrop(true);
          return;
        }

        let fallbackResults: api.BigCommerceCustomer[] = [];
        if (canSearchBC) {
          try {
            fallbackResults = await api.searchBigCommerceCustomers(customerSearch);
            if (fallbackResults.length > 0) {
              await saveLocalPosCustomers(fallbackResults.map((customer) => toLocalPosCustomer(customer)));
            }
          } catch (fallbackError) {
            if (mirrorError) throw fallbackError;
          }
        } else if (mirrorError) {
          throw mirrorError;
        }
        ...
        if (!isCurrent()) return;
        const finalResults = fallbackResults.length > 0 ? fallbackResults : localResults;
        setCustomerResults(finalResults);
        setShowCustomerDrop(finalResults.length > 0);
        ...
        toast({
          ...
          description: e.message || "Could not reach the customer directory.",
          variant: "destructive",
        });
      } finally {
        if (isCurrent()) setIsCustomerSearching(false);
      }
    }, 300);
    return () => {
      if (customerDebRef.current) clearTimeout(customerDebRef.current);
    };
  }, [customerSearch, isOfflineMode, selectedCustomer, currentUser, canSearchBC, toast]);
  ...
  const handleSelectCustomer = (c: api.BigCommerceCustomer) => {
    ...
    if (isOfflineMode) {
      toast({
        title: "Customer loaded from device",
        description: "Addresses, live store credit, pricing, and checkout require a connection.",
      });
    }
  };
