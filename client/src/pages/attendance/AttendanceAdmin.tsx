...
function Overview() {
  ...
  const kpis = [
    ...
    ["Employees", data?.kpis?.employees ?? "—", Users, "text-emerald-600"],
    ["Currently Working", data?.kpis?.currentlyWorking ?? "—", CheckCircle2, "text-green-600"],
    ["Needs Review", data?.kpis?.needsReview ?? "—", AlertTriangle, "text-orange-600"],
    ["Missing Time Out", data?.kpis?.missingTimeOut ?? "—", Clock3, "text-red-600"],
  ] as const;
  return <AdminShell activeTab="overview">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <PeriodButtons value={period} onChange={setPeriod} />
      <div className="text-right text-xs text-slate-400">
        {data?.payPeriod && <p className="font-semibold text-slate-600">Pay period · {data.payPeriod.start} → {data.payPeriod.end}</p>}
        <p>{data?.period?.from} → {data?.period?.to}{data?.payPeriod?.payday ? ` · Payday ${data.payPeriod.payday}` : ""}</p>
      </div>
    </div>
    <div className="mt-4 grid grid-cols-2 gap-3 lg:grid-cols-5">
      {kpis.map(([label, value, Icon, color]) => {
        const actionable = label === "Needs Review" || label === "Missing Time Out";
        return <button key={label} type="button" disabled={!actionable} onClick={() => actionable && (window.location.href = `/attendance/logs?reviewStatus=${label === "Needs Review" ? "needs_review" : "missing_time_out"}`)} className={`text-left ${actionable ? "transition hover:-translate-y-0.5" : ""}`}>
          <Card className="h-full rounded-xl border-slate-200 shadow-sm"><CardContent className="p-4"><Icon className={`h-4 w-4 ${color}`} /><p className="mt-3 text-xl font-bold text-slate-800">{value}</p><p className="mt-1 text-[11px] text-slate-400">{label}</p></CardContent></Card>
        </button>;
      })}
    </div>
...
function LogDetail({ id, onClose }: { id: number; onClose: () => void }) {
  ...
  const data = query.data;
  if (!data) return null;
  const { attendance, employee, checkpoints = [], audit = [] } = data;
...
function Logs() {
  ...
  const [selectedId, setSelectedId] = useState<number | null>(() => {
    const value = new URLSearchParams(location.split("?")[1] ?? "").get("record");
