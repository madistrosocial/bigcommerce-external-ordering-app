...
export function MarketingDashboard() {
  ...
  return <PageShell title="Marketing Dashboard" subtitle="Plan and measure customer communications" action={<Button onClick={() => setLocation("/marketing/campaigns/new")}><Plus className="mr-2 h-4 w-4" /> New campaign</Button>}>
    ...
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      ...
      <StatCard label="Open rate" value={`${d.openRate ?? 0}%`} caption={`${d.clickRate ?? 0}% click rate`} icon={BarChart3} />
    </div>
    ...
    <div className="rounded-xl border border-blue-100 bg-blue-50/60 p-4 text-sm text-blue-900"><div className="flex gap-3"><Target className="mt-0.5 h-5 w-5 shrink-0 text-blue-600" /><div><p className="font-semibold">Marketing foundation</p><p className="mt-1 text-blue-800/80">Audiences use your existing CRM customer records. Delivery, opens, and clicks stay at zero until a real sending or tracking integration is connected.</p></div></div></div>
  </PageShell>;
}
...
function CampaignDetail({ id }: { id: number }) {
  ...
  return <PageShell title={campaign.name} subtitle={campaign.internal_description || "Campaign details"} action={<div className="flex gap-2"><Button variant="outline" onClick={() => setLocation("/marketing/campaigns")}><ArrowLeft className="mr-2 h-4 w-4" /> Back</Button>{hasPermission("marketing", "edit") && campaign.status !== "sent" && <Button onClick={() => setLocation(`/marketing/campaigns/${id}/edit`)}>Edit campaign</Button>}</div>}>
    ...
    <div className="grid gap-5 lg:grid-cols-[1.35fr_1fr]"><div className="space-y-5">
      ...
      <section className="rounded-xl border bg-white p-5 shadow-sm"><h2 className="mb-4 font-semibold text-slate-900">Analytics</h2><div className="grid grid-cols-2 gap-3">{[["Planned", campaign.recipient_count], ["Sent", campaign.sent_count], ["Delivered", campaign.delivered_count], ["Opened", campaign.opened_count], ["Clicked", campaign.clicked_count]].map(([label, value]) => <div key={String(label)} className="rounded-lg bg-slate-50 p-3"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-xl font-bold text-slate-800">{value ?? 0}</p></div>)}</div><p className="mt-4 text-xs text-slate-400">Tracking is unavailable until a delivery integration is connected.</p></section>
      ...
      {hasPermission("marketing", "send") && <section className="rounded-xl border bg-white p-5 shadow-sm"><h2 className="mb-3 font-semibold text-slate-900">Workflow</h2><div className="flex flex-wrap gap-2">{next && <Button size="sm" onClick={() => statusMutation.mutate(next)} disabled={statusMutation.isPending}>{next === "ready" ? "Mark ready" : next === "scheduled" ? "Schedule campaign" : "Pause campaign"}</Button>}{campaign.status === "paused" && <Button size="sm" variant="outline" onClick={() => statusMutation.mutate("ready")}>Resume</Button>}{hasPermission("marketing", "delete") && campaign.status !== "sent" && <Button size="sm" variant="ghost" className="text-red-600 hover:text-red-700" onClick={() => { if (window.confirm("Delete this campaign?")) deleteMutation.mutate(); }}><Trash2 className="mr-1 h-4 w-4" /> Delete</Button>}</div><p className="mt-3 text-xs text-slate-400">Sending itself is intentionally not performed by Phase 1.</p></section>}
    </div></div>
  </PageShell>;
}
...
function CampaignEditor({ id }: { id?: number }) {
  ...
  return <PageShell title={editing ? "Edit campaign" : "New campaign"} subtitle="Build the campaign now; sending is not performed in Phase 1." action={<Button variant="outline" onClick={() => setLocation(editing ? `/marketing/campaigns/${id}` : "/marketing/campaigns")}><ArrowLeft className="mr-2 h-4 w-4" /> Cancel</Button>}>
    <div className="grid gap-5 lg:grid-cols-[1.35fr_1fr]"><div className="space-y-5">
      ...
      <div className="flex flex-wrap justify-end gap-2"><Button variant="outline" onClick={() => saveMutation.mutate("draft")} disabled={saveMutation.isPending}>Save draft</Button>{hasPermission("marketing", "send") && <Button onClick={() => saveMutation.mutate("draft")} disabled={saveMutation.isPending}>{saveMutation.isPending ? "Saving…" : "Save & review"}</Button>}</div>
    </div></div>
  </PageShell>;
}
