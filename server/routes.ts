...
async function refreshBcCustomersForSearch(
  storeHash: string,
  token: string,
): Promise<CachedBcCustomer[]> {
  if (bcCustomerSearchInFlight?.storeHash === storeHash) {
    return bcCustomerSearchInFlight.promise;
  }
  ...
  const promise = (async () => {
    ...
    const customers: CachedBcCustomer[] = [];
    let page = 1;

    while (true) {
      const pageResults = await Promise.all(
        Array.from({ length: BC_CUSTOMER_DIRECTORY_PAGE_CONCURRENCY }, async (_, offset) => {
          const pageNumber = page + offset;
          const response = await fetch(
            `https://api.bigcommerce.com/stores/${storeHash}/v2/customers?limit=250&page=${pageNumber}`,
            { headers },
          );
          if (!response.ok) {
            throw new Error(`BigCommerce customer directory error (${response.status})`);
          }
          return { pageNumber, data: await response.json() };
        }),
      );
      pageCount += pageResults.length;

      let reachedEnd = false;
      for (const { data } of pageResults.sort((a, b) => a.pageNumber - b.pageNumber)) {
      ...
      if (reachedEnd) break;
      page += BC_CUSTOMER_DIRECTORY_PAGE_CONCURRENCY;
    }

    bcCustomerSearchCache = {
      storeHash,
      loadedAt: Date.now(),
      customers,
    };
  ...
  bcCustomerSearchInFlight = { storeHash, promise };
  void promise
    .then(() => {
      console.info("[BC customer directory timing]", JSON.stringify({
        pages: pageCount,
    ...
    .catch((error: any) => {
      console.warn("[BC customer directory timing]", JSON.stringify({
        pages: pageCount,
        totalMs: Math.round(performance.now() - refreshStartedAt),
        state: "failed",
        error: error.message,
  ...
  try {
    ...
    if (bcCustomerSearchInFlight?.promise === promise) {
      bcCustomerSearchInFlight = null;
    }
  }
}

async function getBcCustomersForSearch(
  storeHash: string,
  token: string,
): Promise<BcCustomerDirectoryResult> {
  const now = Date.now();
  if (bcCustomerSearchCache?.storeHash === storeHash) {
    if (now - bcCustomerSearchCache.loadedAt < BC_CUSTOMER_SEARCH_CACHE_TTL_MS) {
      return { customers: bcCustomerSearchCache.customers, state: "fresh" };
    }

    // Serve the last complete directory while refreshing it in the
    // background. This removes the five-minute cache-expiry latency from the
    // search response without pretending that the stale data is fresh.
    void refreshBcCustomersForSearch(storeHash, token).catch((error: any) => {
      console.warn("[BC customer search] Background directory refresh failed:", error.message);
    });
    return { customers: bcCustomerSearchCache.customers, state: "stale" };
  }

  const customers = await refreshBcCustomersForSearch(storeHash, token);
  return { customers, state: "cold" };
}
...
export async function registerRoutes(
  ...
  await db.execute(sql.raw(`
    ...
    DO $$
    BEGIN
      IF to_regclass('public.bc_order_line_items') IS NOT NULL THEN
        ALTER TABLE bc_order_line_items
          ADD COLUMN IF NOT EXISTS price_source text NOT NULL DEFAULT 'legacy';
      END IF;
    END $$;
  `));

  // Attendance sessions originally allowed unlimited completed sessions per
  // workday. Keep the existing records intact while adding the fields needed
  // to enforce one normal shift plus one explicitly approved second shift.
  await db.execute(sql.raw(`
    DO $$
    BEGIN
  ...
  // POS customer search — local mirror first, with an on-demand incremental
  // sync only when the mirror has no match. Live BC lookups for addresses,
  // store credit, and price-list assignment happen after selection.
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        let rows = await storage.searchCrmCustomersForPos(q, 10);
        if (rows.length === 0) {
          try {
            await runIncrementalCustomerSync({ onDemand: true });
            rows = await storage.searchCrmCustomersForPos(q, 10);
          } catch (syncError: any) {
            console.warn("[POS customer search] On-demand mirror sync failed:", syncError.message);
          }
        }

        let customers = rows.map(toPosCustomerResponse);
        if (customers.length === 0) {
          try {
            customers = await searchPosCustomersInBigCommerce(q);
          } catch (fallbackError: any) {
            console.warn("[POS customer search] BigCommerce fallback failed:", fallbackError.message);
          }
        }
        res.json(customers);
      } catch (error: any) {
        console.error("[POS customer search] Caught error:", error.message);
        res.status(500).json({ error: "Customer search failed" });
      }
    },
  );

  // PostgreSQL-only POS search for the client fallback chain. Unlike the
  // public POS search route above, this never calls BigCommerce directly.
  app.get(
    "/api/pos/customers/search-mirror",
    requireAuth,
    async (req, res) => {
      try {
        const q = String(req.query.query ?? "").trim();
        ...
        if (rows.length === 0) {
          try {
            ...
            console.warn("[POS mirror search] On-demand mirror sync failed:", syncError.message);
          }
        }
        res.json(rows.map(toPosCustomerResponse));
      } catch (error: any) {
        console.error("[POS mirror search] Caught error:", error.message);
        res.status(500).json({ error: "Customer mirror search failed" });
      }
    },
  );

  // Lightweight mirror snapshot for the device-local POS customer directory.
  // Pages are bounded and may be restricted to rows updated after a cursor.
  app.get(
    "/api/pos/customers/snapshot",
    requireAuth,
    async (req, res) => {
      try {
        ...
        const rawCursor = req.query.cursor == null ? undefined : Number(req.query.cursor);
        const cursor = rawCursor == null || !Number.isFinite(rawCursor) ? undefined : Math.floor(rawCursor);
        const rawSince = String(req.query.updatedSince ?? "").trim();
        const rawUntil = String(req.query.updatedUntil ?? "").trim();
        const updatedSince = rawSince ? new Date(rawSince) : undefined;
        const updatedUntil = rawUntil ? new Date(rawUntil) : new Date();
        if (updatedSince && Number.isNaN(updatedSince.getTime())) {
          return res.status(400).json({ error: "Invalid updatedSince" });
        }
        if (Number.isNaN(updatedUntil.getTime())) {
          return res.status(400).json({ error: "Invalid updatedUntil" });
        }

        const page = await storage.getPosCustomerDirectoryPage({
          updatedSince,
          updatedUntil,
          cursor,
          limit,
        });
        const { storeHash } = await getBcConfig();
        res.json({
          customers: page.rows.map(toPosCustomerResponse),
          nextCursor: page.nextCursor,
          hasMore: page.hasMore,
          updatedUntil: updatedUntil.toISOString(),
          storeScope: storeHash ? String(storeHash) : "unconfigured",
        });
      } catch (error: any) {
        console.error("[POS customer snapshot] Caught error:", error.message);
        res.status(500).json({ error: "Customer snapshot failed" });
      }
    },
  );

  // BigCommerce customer search (legacy/non-POS consumers)
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        // BigCommerce's v3 customer endpoint supports name/email filters, but
        // this store rejects company:like and phone:like. Use the v3 filter
        // where available, then search the cached v2 customer directory locally
        // for company and phone matches.
        const q = String(query).trim();
        const searchStartedAt = performance.now();
        const isEmailSearch = q.includes("@");
        let directoryError = false;
        const [nameOrEmailResult, directoryResult] = await Promise.all([
          (async () => {
            const v3StartedAt = performance.now();
            const filterParam = isEmailSearch ? "email:in" : "name:like";
            const bcUrl =
              `https://api.bigcommerce.com/stores/${storeHash}/v3/customers?` +
              `${filterParam}=${encodeURIComponent(q)}&limit=10`;
            ...
            return {
              ...
              elapsedMs: Math.round(performance.now() - v3StartedAt),
            };
          })(),
          isEmailSearch
            ? Promise.resolve<BcCustomerDirectoryResult>({ customers: [], state: "fresh" })
            : getBcCustomersForSearch(String(storeHash), String(token)).catch((error: any) => {
                directoryError = true;
                console.warn("[BC customer search] Local company/phone fallback failed:", error.message);
                return { customers: [], state: "cold" as const };
              }),
        ]);
        const nameOrEmailRows = nameOrEmailResult.rows;
        const localRows = directoryResult.customers.filter((customer) => {
          const queryLower = q.toLowerCase();
          const queryDigits = q.replace(/\D/g, "");
          const company = String(customer.company || "").toLowerCase();
          const phoneDigits = String(customer.phone || "").replace(/\D/g, "");
          return company.includes(queryLower) ||
            (queryDigits.length >= 3 && phoneDigits.includes(queryDigits));
        });
        if (!isEmailSearch) {
          res.setHeader(
            "X-BC-Customer-Directory",
            directoryError
              ? "unavailable"
              : directoryResult.state === "stale"
                ? "stale-cache-refreshing"
                : directoryResult.state,
          );
        }
        console.info("[BC customer search timing]", JSON.stringify({
          queryType: isEmailSearch ? "email" : "name-company-phone",
          v3Ms: nameOrEmailResult.elapsedMs,
          directoryState: isEmailSearch ? "not-needed" : directoryError ? "unavailable" : directoryResult.state,
          directoryWaitMs: isEmailSearch ? 0 : Math.round(performance.now() - searchStartedAt - nameOrEmailResult.elapsedMs),
        ...
        const rowsByFilter = [nameOrEmailRows, localRows];
  ...
  // Printable customer order form matching the supplied workbook layout.
  // This intentionally stays self-contained so email attachments do not need
  // a template file or a second spreadsheet dependency at runtime.
  function buildOrderFormXlsx(input: {
    customer: any;
    products: Array<{
      brand: string;
      name: string;
      sku: string;
      variants: Array<{ sku: string; stock_level: number; variant: string }>;
    }>;
    generatedAt: Date;
  }): Buffer {
    ...
    const cell = (ref: string, value: unknown, style = 0) => {
      if (value === null || value === undefined || value === "") {
        return `<c r="${ref}" s="${style}"/>`;
      }
      return `<c r="${ref}" s="${style}" t="inlineStr"><is><t>${escapeXml(value)}</t></is></c>`;
    };
    const row = (rowNumber: number, cells: string[], style = 0, height?: number) =>
      `<row r="${rowNumber}"${height ? ` ht="${height}" customHeight="1"` : ""}>${cells.map((value, index) => cell(`${columnLetter(index)}${rowNumber}`, value, style)).join("")}</row>`;
    const merge = (range: string) => `<mergeCell ref="${range}"/>`;
    const customerName = [input.customer.first_name, input.customer.last_name].filter(Boolean).join(" ")
      || input.customer.company
      || `Customer ${input.customer.id}`;
    const businessName = String(input.customer.company ?? "");
    const dateLabel = input.generatedAt.toLocaleDateString("en-US");
    const sheetRows: string[] = [];
    const merges: string[] = ["A1:H1", "G3:H3", "G4:H4"];

    sheetRows.push(row(1, ["Order Form", "", "", "", "", "", "", ""], 1, 30));
    sheetRows.push(row(2, ["", "", "", "", "", "", "", ""], 0, 10));
    sheetRows.push(row(3, ["", "Customer Name:", businessName ? `${customerName} · ${businessName}` : customerName, "", "Date:", dateLabel, "PO #:", ""], 2, 20));
    sheetRows.push(row(4, ["", "Phone/Email:", String(input.customer.email ?? ""), "", "Delivery/Pickup:", "", "Requested Date:", ""], 2, 20));
    sheetRows.push(row(5, ["", "", "", "", "", "", "", ""], 0, 10));
    sheetRows.push(row(6, ["", "", "", "", "", "", "", ""], 0, 10));
    sheetRows.push(row(7, ["", "Brand", "Product / Device", "Variant / Flavor", "SKU", "Availability", "Qty", "Notes"], 3, 23));

    let currentRow = 8;
    for (const product of input.products) {
      const groupLabel = [product.brand, product.name].filter(Boolean).join(" — ") || product.name;
      sheetRows.push(row(currentRow, ["", groupLabel, "", "", "", "", "", ""], 4, 21));
      merges.push(`B${currentRow}:H${currentRow}`);
      currentRow++;
      for (const variant of product.variants) {
        const status = Number(variant.stock_level) > 0 ? "Available" : "OOS";
        const values = ["", product.brand, product.name, variant.variant || "Default", variant.sku || product.sku, status, "", ""];
        const styles = [0, 5, 5, 5, 5, status === "Available" ? 7 : 8, 6, 6];
        sheetRows.push(`<row r="${currentRow}" ht="20" customHeight="1">${values.map((value, index) => cell(`${columnLetter(index)}${currentRow}`, value, styles[index])).join("")}</row>`);
        currentRow++;
      }
      currentRow++;
    }
    const footerRow = currentRow;
    sheetRows.push(row(footerRow, ["Availability reflects live BigCommerce inventory at the time this form was generated. Enter order quantities in the Qty column.", "", "", "", "", "", "", ""], 9, 32));
    ...
    const stylesXml = `<?xml version="1.0" encoding="UTF-8"?>
      <styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
        <numFmts count="0"/>
        <fonts count="7">
          <font><sz val="10"/><name val="Arial"/><color rgb="FF111827"/></font>
          <font><b/><sz val="18"/><name val="Arial"/><color rgb="FFFFFFFF"/></font>
          <font><b/><sz val="11"/><name val="Arial"/><color rgb="FF111827"/></font>
          <font><b/><sz val="11"/><name val="Arial"/><color rgb="FFFFFFFF"/></font>
          <font><b/><sz val="11"/><name val="Arial"/><color rgb="FF111827"/></font>
          <font><b/><sz val="10"/><name val="Arial"/><color rgb="FF134F5C"/></font>
        ...
        <fills count="9">
          ...
          <fill><patternFill patternType="solid"><fgColor rgb="FFF1F8FF"/><bgColor indexed="64"/></patternFill></fill>
        ...
        <borders count="3">
          ...
          <border><left style="medium"><color rgb="FF9CA3AF"/></left><right style="medium"><color rgb="FF9CA3AF"/></right><top style="medium"><color rgb="FF9CA3AF"/></top><bottom style="medium"><color rgb="FF9CA3AF"/></bottom><diagonal/></border>
        </borders>
        <cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs>
        <cellXfs count="9">
          <xf numFmtId="0" fontId="0" fillId="0" borderId="0" applyAlignment="1"><alignment vertical="center"/></xf>
          <xf numFmtId="0" fontId="1" fillId="2" borderId="2" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf>
          ...
          <xf numFmtId="0" fontId="4" fillId="5" borderId="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf>
          <xf numFmtId="0" fontId="5" fillId="6" borderId="1" applyAlignment="1"><alignment vertical="center" wrapText="1"/></xf>
          <xf numFmtId="0" fontId="6" fillId="8" borderId="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf>
          <xf numFmtId="0" fontId="6" fillId="7" borderId="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf>
          <xf numFmtId="0" fontId="6" fillId="6" borderId="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf>
    ...
    const sheetXml = `<?xml version="1.0" encoding="UTF-8"?>
      <worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
        ...
        <mergeCells count="${merges.length}">${merges.map(merge).join("")}</mergeCells>
    ...
    const files = new Map<string, string>([
      ...
      ["_rels/.rels", `<?xml version="1.0" encoding="UTF-8"?><Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships"><Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument" Target="xl/workbook.xml"/></Relationships>`],
      ["xl/workbook.xml", `<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Order Form" sheetId="1" r:id="rId1"/></sheets></workbook>`],
    ...
    for (const [name, content] of files) {
      ...
      const crc = crmCrc32(dataBuffer);
      const local = Buffer.allocUnsafe(30 + nameBuffer.length);
      local.writeUInt32LE(0x04034b50, 0); local.writeUInt16LE(20, 4); local.writeUInt16LE(0, 6); local.writeUInt16LE(0, 8);
      local.writeUInt16LE(0, 10); local.writeUInt16LE(0, 12); local.writeUInt32LE(crc, 14); local.writeUInt32LE(dataBuffer.length, 18);
      local.writeUInt32LE(dataBuffer.length, 22); local.writeUInt16LE(nameBuffer.length, 26); local.writeUInt16LE(0, 28); nameBuffer.copy(local, 30);
      localParts.push(local, dataBuffer);
      const central = Buffer.allocUnsafe(46 + nameBuffer.length);
      ...
      central.writeUInt32LE(dataBuffer.length, 20); central.writeUInt32LE(dataBuffer.length, 24); central.writeUInt16LE(nameBuffer.length, 28);
      central.writeUInt16LE(0, 30); central.writeUInt16LE(0, 32); central.writeUInt16LE(0, 34); central.writeUInt16LE(0, 36);
      central.writeUInt32LE(0, 38); central.writeUInt32LE(offset, 42); nameBuffer.copy(central, 46);
    ...
    end.writeUInt16LE(count, 8); end.writeUInt16LE(count, 10); end.writeUInt32LE(centralBuffer.length, 12);
  ...
  function toPosCustomerResponse(customer: {
    ...
    return {
      id: customer.bigcommerce_customer_id,
      ...
      customer_group_name: customer.customer_group_name ?? undefined,
      store_credit_amount: 0,
    };
  }

  async function searchPosCustomersInBigCommerce(query: string): Promise<ReturnType<typeof toPosCustomerResponse>[]> {
    const { storeHash, token } = await getBcConfig();
    if (!storeHash || !token) throw new Error("BigCommerce not configured");
    const q = query.trim();
    const searchStartedAt = performance.now();
    const isEmailSearch = q.includes("@");
    const headers = {
    ...
    const [nameOrEmailResult, directoryResult] = await Promise.all([
      (async () => {
        ...
        const filterParam = isEmailSearch ? "email:in" : "name:like";
        const response = await fetch(
          `https://api.bigcommerce.com/stores/${storeHash}/v3/customers?${filterParam}=${encodeURIComponent(q)}&limit=10`,
          { headers },
        );
      ...
      isEmailSearch
        ...
        : getBcCustomersForSearch(String(storeHash), String(token)).catch((error: any) => {
            directoryError = true;
            console.warn("[POS customer search] BigCommerce directory fallback failed:", error.message);
            return { customers: [], state: "cold" as const };
          }),
    ]);
    const nameOrEmailRows = nameOrEmailResult.rows;
    const localRows = directoryResult.customers.filter((customer) => {
      ...
      const company = String(customer.company || "").toLowerCase();
      const phoneDigits = String(customer.phone || "").replace(/\D/g, "");
      return company.includes(queryLower) || (queryDigits.length >= 3 && phoneDigits.includes(queryDigits));
    });
    console.info("[POS customer search timing]", JSON.stringify({
      queryType: isEmailSearch ? "email" : "name-company-phone",
      v3Ms: nameOrEmailResult.elapsedMs,
      directoryState: isEmailSearch ? "not-needed" : directoryError ? "unavailable" : directoryResult.state,
      directoryWaitMs: isEmailSearch ? 0 : Math.round(performance.now() - searchStartedAt - nameOrEmailResult.elapsedMs),
    ...
    for (let index = 0; rows.length < 10; index += 1) {
      let added = false;
      for (const filterRows of [nameOrEmailRows, localRows]) {
        const customer = filterRows[index];
        if (!customer || seenIds.has(customer.id)) continue;
        seenIds.add(customer.id);
        rows.push(customer);
    ...
    return rows.map((customer) => toPosCustomerResponse({
  ...
  let incrementalCustomerSyncInFlight: Promise<number> | null = null;
  let lastOnDemandCustomerSyncAt = 0;

  async function runIncrementalCustomerSync(options: { onDemand?: boolean } = {}): Promise<number> {
    const onDemand = options.onDemand === true;
    const now = Date.now();
    if (onDemand && now - lastOnDemandCustomerSyncAt < 30_000) return 0;
    if (incrementalCustomerSyncInFlight) return incrementalCustomerSyncInFlight;
    if (onDemand) lastOnDemandCustomerSyncAt = now;

    incrementalCustomerSyncInFlight = (async () => {
      const { storeHash, token } = await getBcConfig();
      if (!storeHash || !token) throw new Error("BigCommerce not configured");
      ...
      await storage.setSetting("crm_last_customer_incremental_sync", syncTime);
      await storage.setSetting("crm_last_customer_sync", syncTime);
      return synced;
    })();

    try {
      return await incrementalCustomerSyncInFlight;
    } finally {
      incrementalCustomerSyncInFlight = null;
    }
  }
  ...
  app.post("/api/crm/sync/customers/incremental", requireAuth, async (_req, res) => {
    try {
      const synced = await runIncrementalCustomerSync();
      res.json({ success: true, synced });
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });
  ...
  async function fetchAndStoreLineItems(
    ...
    const newItems = bcItems!.map((item: any) => {
      const options = Array.isArray(item.product_options)
        ...
        : null;
      const resolvedPrice = resolveBigCommerceOrderLinePrice(item);
      return {
        ...
        base_price: resolvedPrice.amount,
  ...
  // GET /api/crm/sync/line-items/price-audit — distinguish checkout prices,
  // legitimate zero-price promotions, missing prices, and pre-audit legacy rows.
  app.get("/api/crm/sync/line-items/price-audit", requireAuth, async (req, res) => {
    try {
      const user = (req as any).authUser;
      if (user?.role !== "admin") return res.status(403).json({ error: "Forbidden" });
      res.json(await storage.getBcOrderLineItemsPriceAudit());
    } catch (e: any) {
      res.status(500).json({ error: e.message });
    }
  });
  ...
  app.post("/api/attendance/start", requirePermission("attendance", "clock"), async (req, res) => {
    try {
      ...
      const firstSession = todaySessions.find(session => session.session_number === 1) ?? todaySessions[0];
      const sessionNumber = todaySessions.length === 0 ? 1 : 2;
  ...
  app.post("/api/attendance/admin/allow-second-session", requirePermission("attendance", "audit"), async (req, res) => {
    try {
      const user = (req as any).authUser;
      ...
      const target = await storage.getAttendanceById(attendanceId);
      if (!target) return res.status(404).json({ error: "Attendance record not found." });
      if (target.session_number !== 1) return res.status(409).json({ error: "Only the first session can authorize a second session." });
      const reason = String(req.body?.reason ?? "").trim().slice(0, 2000);
      if (!reason) return res.status(400).json({ error: "A reason is required to approve a second session." });
      const approved = await storage.approveAttendanceSecondSession(target.user_id, target.work_date, user.id);
      if (!approved) {
        return res.status(409).json({ error: "A second session cannot be approved while the first session is active, after another session exists, or after approval was already granted." });
      }
      await storage.createAttendanceAuditLog({
        attendance_id: approved.id,
        actor_user_id: user.id,
        action: "second_session_approved",
        changed_field: "second_session_approved",
        old_value: false,
        new_value: true,
        reason,
      });
  ...
  app.get("/api/attendance/admin/overview", requirePermission("attendance", "view_dashboard"), async (req, res) => {
    try {
      ...
      const today = dateOnlyInTimeZone(new Date(), await getCompanyTimezone(storage));
  ...
  app.patch("/api/attendance/admin/logs/:id/correction", requirePermission("attendance", "manage"), async (req, res) => {
    try {
      ...
      const parseCorrectionTime = (value: unknown) => {
        const raw = String(value ?? "").trim();
        if (!raw) return null;
        const hasExplicitTimezone = /(?:Z|[+-]\d{2}:?\d{2})$/i.test(raw);
        return hasExplicitTimezone ? new Date(raw) : parseDateTimeLocal(raw, companyTimezone);
      };
      const hasTimeIn = req.body?.time_in !== undefined && req.body?.time_in !== null && String(req.body.time_in).trim() !== "";
      const hasTimeOut = req.body?.time_out !== undefined && req.body?.time_out !== null && String(req.body.time_out).trim() !== "";
      const nextTimeIn = hasTimeIn ? parseCorrectionTime(req.body.time_in) : attendance.time_in;
      const nextTimeOut = hasTimeOut ? parseCorrectionTime(req.body.time_out) : attendance.time_out;
      if ((hasTimeIn && (!nextTimeIn || Number.isNaN(nextTimeIn.getTime()))) || (hasTimeOut && (!nextTimeOut || Number.isNaN(nextTimeOut.getTime())))) {
    ...
    try {
      ...
      for (const row of result.rows) {
        const current = byEmployee.get(row.user_id) ?? {
          user_id: row.user_id,
          employee_name: row.employee_name || row.employee_username || "Employee",
          total_seconds: 0,
          days: 0,
          missing: 0,
          review: 0,
      ...
      const rows = Array.from(byEmployee.values()).sort((a, b) => String(a.employee_name).localeCompare(String(b.employee_name)));
      res.json({
        from,
        to,
        summary: {
  ...
  app.get("/api/reports/product-search", requirePermission("reporting_sales"), async (req, res) => {
    try {
      ...
      // 2. Search the line-item mirror for variant SKUs. Product-name searches
      // only fall back to the mirror when the local catalog has no match; this
      // avoids scanning hundreds of thousands of historical line items for
      // every ordinary product-name keystroke.
      const looksLikeSku = /^[a-z0-9._/-]+$/i.test(q) && !/\s/.test(q);
      const lineItemHitsRaw = looksLikeSku || productHits.length === 0
        ? await storage.searchLineItemsByQuery(q, 40, { skuOnly: looksLikeSku })
        : [];
  ...
  async function fetchLiveSalesReportInventory(
    keys: SalesReportInventoryKey[],
  ): Promise<Map<string, number>> {
    const productIds = [...new Set(keys.map(key => Number(key.bc_product_id)).filter(Number.isFinite))];
    ...
    for (let index = 0; index < productIds.length; index += 100) {
      ...
      const response = await fetch(
        `https://api.bigcommerce.com/stores/${storeHash}/v3/catalog/products?id:in=${ids.join(",")}&include=variants&limit=250`,
        { headers },
      );
      if (!response.ok) {
        throw new Error(`BigCommerce inventory lookup failed (${response.status})`);
      }
  ...
  function applyLiveSalesReportInventory(
  ...
  app.get("/api/reports/sales", requirePermission("reporting_sales"), async (req, res) => {
    try {
       ...
       const bcStatusFilters = String(bcStatusFilter ?? "")
         .split(",")
      ...
      const opts = {
        ...
        skuFilter: skuFilter || undefined,
         bcStatusFilter: bcStatusFilters.length > 0 ? bcStatusFilters : undefined,
      ...
      if (view === "summary") {
        const inventory = await fetchLiveSalesReportInventory(
  ...
  app.get("/api/reports/sales/stats", requirePermission("reporting_sales"), async (req, res) => {
    try {
      ...
      const inventoryKeys = await storage.getSalesReportInventoryKeys({
        dateFrom, dateTo, bcProductIds: resolvedIds,
        skuFilter: skuFilter || undefined,
        bcStatusFilter: bcStatusFilters.length > 0 ? bcStatusFilters : undefined,
      });
