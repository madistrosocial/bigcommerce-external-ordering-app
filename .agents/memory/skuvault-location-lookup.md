...
## Live audit quantities (getProducts)

Use `/products/getProducts` with `ProductSKUs` param for the audit dialog's On Hand / Pending / Available display.
- `QuantityOnHand`    → On Hand    (total physical; matches SKUVault UI)
- `QuantityPending`   → Pending    (committed across ALL connected sales channels — SKUVault native)
- `QuantityAvailable` → Available  (what's left to sell)

**Why NOT getInventoryByLocation for live display:** sums ALL physical bins including virtual channel-allocation bins, over-counting vs what SKUVault's UI shows.
**Why NOT getAvailableQuantities:** ignores the SKU filter — dumps every product, not the requested set.
**Why NOT `Skus` param for getProducts:** SKUVault ignores the filter; use `ProductSKUs` (same rule as getInventoryByLocation).

## Key mistakes to avoid

- Do NOT use `Skus` in getInventoryByLocation or getProducts — must be `ProductSKUs`
...
- Do NOT use getInventoryByLocation for live display quantities — over-counts due to virtual channel bins
