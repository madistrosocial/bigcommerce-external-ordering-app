...
**How to apply:** Use a Zoho Campaigns/Email API transmission product for campaign delivery if the account is enabled for it, verified for the sending domain, and within its published quota. Preserve the app’s audience and suppression checks, use provider-supported limits, and never silently fall back to mailbox SMTP after API failure. The Admin → Zoho form may hold encrypted testing fallbacks, but environment secrets must win independently for each field so production configuration needs no code change.